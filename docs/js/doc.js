...
+(function(window, $) {
    ...
    var openPage = function($section, section, topic) {
        ...
        $pageAttrs.children('.badge-source').toggle(!!section.url).attr('href', 'https://github.com/stbui/stbui/tree/master/' + section.url);
        var lib = section.lib;
        if(lib) {
            ...
            if(isShowCodeBadage) {
                ...
                var srcPrefix = 'https://github.com/stbui/stbui/blob/' + (zuiPkg.version ? ('v' + zuiPkg.version) : 'master') + '/';
                if(lib.srcCount) {
