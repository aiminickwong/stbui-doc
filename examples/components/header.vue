...
<template>
  <div class="headerWrapper">
    ...
    :class="{
      ...
      <div class="container">
        <h1><router-link to="/">Stbui<span>Beta</span></router-link></h1>
        <ul class="nav">
          <li class="nav-item">
            <router-link
              active-class="active"
...
<script>
  export default {
    ...
    watch: {
      '$route.path'(val) {
        this.isHome = val === '/';
        this.headerStyle.backgroundColor = `rgba(243, 115, 39, ${ this.isHome ? '0' : '1' })`;
      }
    },
    mounted() {
      ...
      scroll((direction) => {
        if (this.isHome) {
          ...
          this.$refs.header.style.backgroundColor = `rgba(243, 115, 39, ${ alpha })`;
          return;
        }
        this.headerStyle.backgroundColor = 'rgba(243, 115, 39, 1)';
